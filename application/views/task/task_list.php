           ...
           <div class="col-xs-12">
            <div class="box">
                ...
                <div class="box-body table-responsive no-padding">
                     <table id="dataTableTaskList" class="table table-striped table-bordered" cellspacing="0" width="100%">
                        <thead>
                            <tr>
...
<div class="modal fade" id="modal_form_addRequestBudgetUser" role="dialog">
    <?php //var_dump($aktifitas); die();
   ?>
    <div class="modal-dialog">
        <div class="modal-content">
                ...
                <form action="#" id="formRequest" class="form-horizontal">
                    <div class="form-body">
                         <div class="form-group">
                                <label class="control-label col-md-3">Tanggal Pengajuan</label>
                                  <div class="col-md-9">
                                   <input class="form-control" type="date" id="tgl_request_user" name="tgl_request" style="width:100%;" value="<?php echo date('Y-m-d'); ?>">
                                   <input class="form-control" type="hidden" name="id" id="id_req_user" style="width:100%;" required>
                                </div>
                        </div>
                        <div class="form-group">
                          <label class="control-label col-md-3">Nominal</label>
                           <div class="col-md-9">
                            <input type="text" name="nominal" class="form-control" id="nominal_user" placeholder="Nominal" style="width:420px;">                       
                           </div>
                      </div>
                      <div class="form-group">
                          <label class="control-label col-md-3">Deskripsi</label>
                           <div class="col-md-9">
                            <textarea class="form-control ckeditor" id="deskripsi_user" name="deskripsi"></textarea>
                           </div>
                      </div>
                    </div>
                </form>
...
var ckeditorUser = CKEDITOR.replace('deskripsi_user',{
      height:'250px'
});
CKEDITOR.disableAutoInline = true;
...
$(document).ready(function() {
    table = $('#dataTableTaskList').DataTable({ 
        "ordering": false,
        "processing": true,
        "serverSide": true,
        "ajax": {
 ...
 function detailRequest(id){
    ...
    $.ajax({
        ...
        success: function(data){
             ...
             CKEDITOR.instances.deskripsi_user.setReadOnly();
        },
 ...
 function detailRequestUser(id){
    ...
    $('.form-group').removeClass('has-error'); 
    $('.help-block').empty(); 
    $('#modal_form_addRequestBudgetUser').modal('show');
    $('.modal-title').text('Detail Request Budget');
    $.ajax({
        ...
        success: function(data){
             ...
             $('#id_req_user').val(data[0].id);
             $('#tgl_request_user').val(data[0].tgl_request);
             $('#nominal_user').val(formatRupiah(data[0].nominal,'Rp. '));
             CKEDITOR.instances.deskripsi.setData( '<p>'+data[0].deskripsi+'</p>' );
          }else{
             $('#id_req_user').val(data[0].id);
             $('#tgl_request_user').val(data[0].tgl_request);
             $('#nominal_user').val(formatRupiah(data[0].nominal,'Rp. '));
             CKEDITOR.instances.deskripsi_user.setData( '<p>'+data[0].deskripsi+'</p>' );
             CKEDITOR.instances.deskripsi_user.setReadOnly();
