      ...
      <div class="box">
        ...
        <div class="box-body">
            <form method="post" action="<?php echo $current_context . 'search'; ?>" class="form-horizontal">
                <?php $key = (object) $this->session->userdata('filter_menu'); ?>
						<!--<div class="col-md-6"><div class="form-group">
								<label class="control-label col-md-5">Men Menu Id</label>
								<div class="col-md-7"><input class="form-control " name="men_menu_id" value="<?php echo $key->men_menu_id; ?>" placeholder="Men Menu Id">
								</div>
							</div>
						</div>-->
						<div class="col-md-6"><div class="form-group">
								<label class="control-label col-md-5">Nama</label>
								<div class="col-md-7"><input class="form-control " name="menu_name" value="<?php echo $key->menu_name; ?>" placeholder="Nama Menu">
								</div>
							</div>
						</div>
        ...
        <div class="box-body table-responsive no-padding">
          <table class="table table-hover" id="table_data">
            ...
            foreach ($menu as $row) {
                ?>
                <tr>
                    <td><input type="checkbox" class="checkboxes"  data-menu_id="<?php echo $row->menu_id; ?>"  /></td>
                    <td><?php echo $i + $offset; ?></td>
							<td><?php echo $row->parent_menu_name; ?></td>
							<td><?php echo $row->menu_name; ?></td>
							<!--<td><?php echo $row->menu_link; ?></td>-->
							<td><?php echo ($row->menu_status == '1')?"Aktif":"Tidak Aktif"; ?></td>
							<!--<td><?php echo $row->menu_ismaster; ?></td>
							<td><?php echo $row->menu_order; ?></td>-->
							<td class="td-btn">
								<!--<a href="<?php echo $current_context . 'detail'  .'/'. $row->menu_id ?>" class="badge bg-yellow"><i class="fa fa-eye fa-fw"></i>lihat</a>-->
								<a href="<?php echo $current_context . 'edit'  .'/'. $row->menu_id ?>" class="badge bg-green"><i class="fa fa-edit fa-fw"></i>ubah</a>
								<!--<a href="#" data-href="<?php echo $current_context . 'delete'  .'/'. $row->menu_id ?>" data-toggle="modal" data-target="#deleteModal"  class="badge bg-red"><i class="fa fa-trash-o fa-fw"></i>hapus</a>-->
							</td>
                </tr>
            <?php $i++;} ?>
          </table>
