...
class Menu extends CI_Controller {
    ...
    private function fetch_input() {
        $data = array('men_menu_id' => (($this->input->post('parent_menu_id') != '--Select--')?$this->input->post('parent_menu_id') : null),
    ...
    public function add() {
        ...
        $obj['menu_id'] = $this->getMaxid();

        if ($this->validate() != false) {
            $this->m_menu->insert_data($obj);
    ...
    public function edit($menu_id) {
        ...
        if ($this->validate() != false) {
            $this->m_menu->update_data($obj, $obj_id);
            redirect(CURRENT_CONTEXT);
        } else {
            $this->preload();
            $this->data['edit'] = true;
    ...
    private function getMaxid(){
        $id=$this->db->query("SELECT MAX(menu_id) as id from menu")->row()->id;
        return $id;
    }
